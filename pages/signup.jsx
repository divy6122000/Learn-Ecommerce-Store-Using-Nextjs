...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const req = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/signup`, {
      method: 'POST', // or 'PUT'
      headers: {
          ...
          <form className="mt-8 space-y-6" method="POST" onSubmit={handleSubmit}>
            ...
            <div className="rounded-md shadow-sm -space-y-px">
              <div>
                ...
                <input onChange={handleOnchange} value={name} id="name" name="name" type="text" autoComplete="name" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-pink-500 focus:border-pink-500 focus:z-10 sm:text-sm" placeholder="Your Name" />
              </div>
              <div>
